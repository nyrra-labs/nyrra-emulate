...
Deploy wiring status: the build emits to `apps/web-svelte/.svelte-kit/cloudflare/` and the repo now ships a minimal `apps/web-svelte/wrangler.toml` plus a `pnpm --filter web-svelte deploy:dry-run` script. The dry run rebuilds the worker and validates that the generated artifact can be packaged for Cloudflare Workers, with the `nodejs_compat` compatibility flag enabled so the SvelteKit server bundle's `node:async_hooks` import does not fail at runtime. There is still no CI workflow that pushes to a real Cloudflare account, and no custom domain, route, or `account_id` is encoded in the wrangler.toml. If a real deploy wrapper or CI workflow lands, or if the production docs domain changes from the value currently encoded in `apps/web-svelte/src/lib/page-metadata.ts`, this README section, `apps/web-svelte/wrangler.toml`, and `apps/web-svelte/src/lib/page-metadata.ts` must be updated together so the documented build path, the deployed hostname, and the metadata `BASE_URL` stay in lockstep.
