...
<!--
	The HTML is rendered at build time by `renderDocsHtmlByHref('/foundry')`
	in `+page.server.ts`, which pulls the raw MDX from the shared
	`docs-source` registry (sibling apps/web Foundry page) and runs it
	...
	Shiki output for fenced code blocks. Input is a build-time-bundled
