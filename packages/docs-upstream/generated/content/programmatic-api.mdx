...
Use emulators as libraries in your tests and scripts. There are two approaches: the `@nyrra/emulate` CLI package (which wraps all services) and the individual `@emulators/*` scoped packages.

## CLI package

The `@nyrra/emulate` package provides a `createEmulator` helper that starts any service by name.
