...
// Generate the TypeScript re-export module
const indexTs = `// Auto-generated by packages/docs-upstream/src/sync.ts
...
export const OG_TYPE = ${JSON.stringify(siteMetadata.OG_TYPE)} as const;
export const OG_LOCALE = ${JSON.stringify(siteMetadata.OG_LOCALE)};
export const TWITTER_CARD = ${JSON.stringify(siteMetadata.TWITTER_CARD)} as const;
...
export const STARTUP_LABEL_OVERRIDES: Readonly<Record<string, string>> = ${JSON.stringify(startupLabelOverrides, null, 2)};
...
export const allDocsPages: NavItem[] = ${JSON.stringify(pages.map((p) => ({ name: p.name, href: p.href })), null, 2)};

export const TOP_SECTION_HREFS: readonly string[] = ${JSON.stringify(topSectionHrefs)};
export const REFERENCE_SECTION_HREFS: readonly string[] = ${JSON.stringify(referenceSectionHrefs)};
export const NAV_LABEL_OVERRIDES: Readonly<Record<string, string>> = ${JSON.stringify(navLabelOverrides, null, 2)};
...
if (!writeOrCheck(path.join(GENERATED, "index.js"), indexTs)) allOk = false;
